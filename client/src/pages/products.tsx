...
export default function Products() {
  const collections = [
    {
      ...
      image: car44,
      description:
        "최고급 독일산 스테인리스 스틸과 이탈리아 전통 기법이 만나 탄생한 완벽한 균형의 프리미엄 나이프 라인입니다.",
      items: [
        '셰프 나이프 (8", 10", 12")',
        "산토쿠 나이프",
        "페어링 나이프",
    ...
    },
    {
      ...
      image: car4,
      description:
        "토스카나산 올리브 나무와 참나무로 제작된 천연 항균 도마는 기능성과 아름다움을 동시에 제공합니다.",
      items: [
        "올리브 우드 도마 (소형, 중형, 대형)",
        "참나무 도마",
        "엔드 그레인 도마",
  ...
  return (
    <div className="py-20 bg-background font-sans" data-testid="page-products">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 mb-16 sm:mb-20">
          {collections.map((collection, index) => (
            <Card
              key={index}
              className="bg-card rounded-lg overflow-hidden shadow-lg touch-target-comfortable font-sans"
            ...
            >
              ...
              <CardContent className="p-4 sm:p-6 lg:p-8">
                <h3
                  className="text-responsive-xl sm:text-responsive-2xl lg:text-responsive-3xl font-sans font-semibold mb-3 sm:mb-4 leading-tight"
                  data-testid={`text-collection-title-${index}`}
                >
                  {collection.title}
                </h3>
                <p
                  className="text-responsive-sm sm:text-responsive-base text-muted-foreground mb-4 sm:mb-6 leading-relaxed"
                  data-testid={`text-collection-description-${index}`}
                >
                ...
                <ul className="space-y-1 sm:space-y-2 text-responsive-xs sm:text-responsive-sm text-muted-foreground">
                  {collection.items.map((item, itemIndex) => (
                    <li
                      key={itemIndex}
                      data-testid={`text-collection-item-${index}-${itemIndex}`}
                    >
                      • {item}
                    </li>
                  ))}
